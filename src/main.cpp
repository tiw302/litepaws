...
int main(int argc, char* argv[]) {
    ...
    // THE MAGIC FOR i3wm:
    // Force X11 to treat this window as an "Override Redirect" window.
    // This makes the Window Manager ignore it for tiling and hiding,
    // so it will STAY across all workspaces (Sticky).
    SDL_SetHint(SDL_HINT_X11_FORCE_OVERRIDE_REDIRECT, "1");
    SDL_SetHint(SDL_HINT_VIDEO_X11_NET_WM_BYPASS_COMPOSITOR, "0");
    SDL_SetHint(SDL_HINT_RENDER_DRIVER, "opengl");

    // Force ARGB 32-bit visual
